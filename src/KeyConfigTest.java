...
/*
 * The KeyConfigTet class extends the MenuTest demo to add a dialog box to configure the keyboard keys. 
 */
...
public class KeyConfigTest extends MenuTest {
	...
	public void init() {
		...
		//create the panel containig the OK button
		...
		//center the dialog
		dialog.setLocation((screen.getWidth() - dialog.getWidth()) / 2, (screen.getHeight() - dialog.getHeight()) /2);
		...
		((JFrame) super.screen.getFullScreenWindow()).getLayeredPane().add(dialog, JLayeredPane.MODAL_LAYER);
	}
	
	/*
	 * Adds a label containing the name of the GameAction and an InputComponent used for changing the mapped keys.
	 */
	...
	/*
	 * Resets the text displayed in each InputComponent, which is the names of the mapped keys.
	 */
	...
	/*
	 * The InputComponent class displays the keys mapped to a particular action and allows the user to change the mapped keys. The user
	 * selects the InputComponent by clicking it, then can press any key or mouse button (including the mouse wheel) to change the 
	 * mapped value.
	 */
	class InputComponent extends JTextField {
		...
		/*
		 * Sets the displayed text of this InputComponent to the names of the mapped keys.
		 */
		...
		/*
		 * Maps the GameAction for this InputComponent to the specified key or mouse action.
		 */
