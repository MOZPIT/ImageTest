...
public class InputManager implements KeyListener, MouseListener, MouseMotionListener, MouseWheelListener {
	...
	public InputManager(Component comp) {
		...
		//register Key and mouse listener
	...
	/*
	 * Sets whether relative mouse mode is on or not. For relative mouse mode, the mouse is "locked" in the cneter of the screen, and only the changed 
	 * in mouse movement is measured. In normal mode, the mouse is free to move about the screen.
	 */
	...
	/*
	 * Maps a GameAction to a specific key. The key codes are defined in java.awt.KeyEvent. If the key already has a GameAction mapped to it,
	 * the new GameAction overwrites it.
	 */
	...
	/*
	 * Maps a GameAction to a specific mouse action. The mouse codes are defined here in InputManager (MOUSE_MOVE_LEFT, MOUSE_BUTTON_1, etc).
	...
	/*
	 * Uses the Robot class to try to position the mouse in the center of the screen. <p>Note that use of the Robot class may not be available
	 * on all platforms.
	 */
	...
	//from the MouseListener interfaces
	...
	//from the MouseMotionListener interface
	public synchronized void moveMoved(MouseEvent e) {
	...
	//from the mMouseWheelListener interface
